...
print("=== [1/6] Initializing Sovereign Enterprise Directory Topology ===")
os.makedirs("layers/base", exist_ok=True)
os.makedirs("layers/models", exist_ok=True)
os.makedirs(".github/workflows", exist_ok=True)

# 1. Dual-Protocol Sovereign Gateway (REST + WebSockets with Redis/Ray PubSub)
with open("sovereign_gateway.py", "w") as f:
    f.write('''from fastapi import FastAPI, WebSocket, WebSocketDisconnect, status
...
# 2. Distributed Memory Grid & Substrate
...
print("=== [2/6] Installing Enterprise Dependencies ===")
...
print("=== [3/6] Cleaning Conflicting Processes & Ports ===")
subprocess.run(["pkill", "-f", "uvicorn"], check=False)
subprocess.run(["pkill", "-f", "streamlit"], check=False)
subprocess.run(["pkill", "-f", "sovereign_"], check=False)

print("=== [4/6] Executing Git Actions & GitHub Enterprise Synchronization ===")
...
subprocess.run(["git", "commit", "-m", "Definitive Master Enterprise Release: Dual-Protocol Mesh Enabled", "--no-verify"], check=False)
subprocess.run(["git", "push", "-u", "origin", "main", "--no-verify"], check=True)

print("=== [5/6] Launching Sovereign Enterprise Microservices & Daemons ===")
...
print("------------------------------------------------------------------------------")
print(" * FastAPI Gateway (REST/WS) : http://127.0.0.1:8000")
print(" * WebSocket Endpoint        : ws://127.0.0.1:8000/ws/{client_id}")
